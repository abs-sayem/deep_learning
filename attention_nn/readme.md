...
**Types of Attention**
* **`Soft Attention`** *The main relevant region(z) consists of different parts of different sub-regions(y).*
###### **`Z = sum(s_n.y_n)`    where, s=probabilities of the sub-regions(y).<br>Soft Attention is deterministic. What "deterministic" means?- A system is said to be deterministic if the application of an action(a), on a state(s), always leads to te same state(s')**
